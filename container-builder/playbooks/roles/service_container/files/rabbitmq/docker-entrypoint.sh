...
if [ -n "$RABBIT_MASTER_HOST" ]; then
  nc -vz con1 5672 > /dev/null
  while [ $? -ne 0 ]; do
    sleep 2
    nc -vz con1 5672
  done
  ...
  rabbitmqctl join_cluster rabbit@$RABBIT_MASTER_HOST
  rabbitmqctl stop
fi
while pgrep -u rabbitmq beam.smp > /dev/null; do sleep 1; done
